...
from app.ingestion.extract import ExtractedPdf, OutlineEntry, TextLine
...
def detect_sections(extracted: ExtractedPdf) -> tuple[list[SectionSpec], Strategy]:
    outline_sections = _from_outline(extracted.outline, extracted.page_count)
    if outline_sections:
        return outline_sections, Strategy.OUTLINE

    heading_sections = _from_headings(extracted.lines, extracted.page_count)
    if heading_sections:
        return heading_sections, Strategy.HEADINGS

    return _flat(extracted.title, extracted.page_count), Strategy.FLAT
...
def _from_outline(outline: list[OutlineEntry], page_count: int) -> list[SectionSpec]:
    ...
    if len(top_level) < MIN_HEADING_DETECTIONS:
        # A book whose outline puts everything one level down (a single "Contents"
        # root, say) still has usable chapter bounds at level 2.
