...
def test_build_embedder_uses_ollama_without_any_key() -> None:
    settings = Settings(embedding_provider="ollama", openai_api_key=None, hf_token=None)

    assert isinstance(build_embedder(settings), OllamaEmbedder)


def test_build_embedder_refuses_an_ollama_dimension_the_column_cannot_hold() -> None:
    settings = Settings(embedding_provider="ollama", embedding_dimensions=1024)
